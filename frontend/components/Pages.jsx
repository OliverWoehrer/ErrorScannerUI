...
function ListElements({ items, onSelect, selectedId, ListView }) {
    /**
     * [INFO]
     * To increase performance of list rendering, we use 'react-virtualized'. It implements
     * virtualized list rendering. Items only get rendered when they are visible. The package
     * provides a wrapper component <AutoSizer> that passes its height and width to its child
     * component. The child component <List> uses the height and width passed from
     * its parent.
     */
    ...
    const rowRenderer = ({ index, key, style }) => {
        ...
        return( // apply 'style' prop generated by react-virtualized
            <div key={key} style={style}>
                <ListView item={item} onClick={onSelect} isSelected={item.id === selectedId} />
            </div>
        );
    };
    ...
    return items.length > 0 ? (
        <AutoSizer>
            {({ height, width }) => ( // AutoSize wrapper passes height and width to List component
                <List
                    height={height}             // Height from AutoSizer
                    width={width}               // Width from AutoSizer
                    rowCount={items.length} // Total number of items
                    rowHeight={60}       // Fixed height of each item in px
...
function ListDetailsLayout({ isLoading, items, Header, ListView, DetailsView }) {
    ...
    // Selected Item Search Param:
    const navigate = useNavigate(); // Add this hook
    const location = useLocation(); // Add this hook
    const [searchParams, setSearchParams] = useSearchParams();
    const selectedId = searchParams.get("id"); // holds ground truth
    const selectedItem = useMemo( // updates currently selected item in details pane based on ground truth
        () => filteredItems.find((it) => String(it.id) === selectedId) ?? null,
    ...
    // Update Selected ID:
    function showDetails(itemID) { setSearchParams({ id:itemID }); }
    function hideDetails() {
        navigate(window.location.pathname, { replace: true }); // set current path to the visible window pathname
    }

    // Remove And Replace Updated Item:
    function onItemUpdate(newItem) {
        console.log(`onItemUpdate(${newItem.id}, ${newItem.solution})`);
        const updateFunction = (prevItems) => prevItems.map(obj => obj.id === newItem.id ? newItem : obj);
        setLocalItems(updateFunction);
        setFilteredItems(updateFunction);
    }
    function onItemDelete(id) {
        const removeFunction = (prevItems) => prevItems.filter(obj => obj.id !== id); // 'filter' returns 'true' on items to keep
        setLocalItems(removeFunction);
        setFilteredItems(removeFunction);
    }
    ...
    const resize = useCallback(
        ...
        [isResizing] // only re-define on resizing
    );
    ...
    const ListPane = (
        <>
                ...
                <mdui-collapse ref={collapseRef}>
                    <mdui-collapse-item trigger="#showFilters">
                        ...
                        <div style={{padding:"0 0.5rem"}}>
                            <ItemFilters items={localItems} onFiltered={setFilteredItems} />
                        </div>
                    </mdui-collapse-item>
                </mdui-collapse>
            ...
            <main>
                <ListElements items={filteredItems} onSelect={showDetails} selectedId={selectedId} ListView={ListView} />
            </main>
        </>
    );
...
export function SettingsPage() {
    function DockerInterfaceForm({endpoint}) {
        ...
        return(
            <mdui-card variant="elevated">
                <TextForm action="/api/form/docker-interface" onSuccess={reloadData}>
                    <header>
                    ...
                    <footer>
                        <mdui-button type="submit">Save changes</mdui-button>
    ...
    function LogScannerForm({endpoint}) {
        ...
        return (
            <mdui-card variant="elevated">
                <TextForm action="/api/form/scanner" onSuccess={reloadData}>
                    <header>
    ...
    function DiskUsageForm({endpoint}) {
        ...
        return(
            <mdui-card variant="elevated">
                <TextForm action={endpoint} onSuccess={reloadData}>
                    <header>
