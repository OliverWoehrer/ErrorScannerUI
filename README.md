...
> TODO: include logo and dataflow diagram
...
4. Include the container into your Docker compose file. The scanner inside the container uses a UNIX socket to connect to the Docker daemon that runs the container. In our case this is Docker Desktop.
    ...
    Example for `compose.yml`:
    ```
    services:
        error_scanner:
            container_name: error_scanner
            environment: # set socket path
            - DOCKER_HOST=unix:///var/run/docker.sock
            volumes: # connect socket paths
    ...
    networks:
        your-docker-network:
            driver: bridge
            external: true # connect to an existing external network
    ```
...
3. The user enabled auto-recording for that category (e.g. error). This means any time a log has that category, it gets automatically recorded. Auto-recording also works for categories that are disabled for logging.

**Auto-Recording**: When a new log is generated, the system checks if a matching record already exists. If a match is found, the system simply updates the existing record’s timestamp instead of creating a duplicate. For a log to match an existing record, they must share the same container, the same category, and similar message strings. 

Since automated string similarity is not always perfect, you can use a **search key** to ensure reliable matching. A search key is a regular expression (Regex) applied to the log message. If the system finds a Regex match within a new log, it is automatically categorized as a match to that record.
...
After successful installation, you can run the application on your machine. The entire development stack is made of four components: Docker scanner, Flask backend server, Vite frontend development server and the Docker Desktop daemon. The frontend server is just so you do not have to recompile the entire frontend for every change, but instead it handles live-updates on the spot.

> TODO: insert a data flow and network sketch about the development stack
...
**VSCode Setup**: In case you use VSCode for development here are some launch configurations for click&play debug directly via VSCode:
```
{
    "name": "Python: Scanner",
    "type": "debugpy",
    ...
    // "args": ["--network", "lognet"],
    "env": {
        "DOCKER_HOST": "unix:///home/oliver/.docker/desktop/docker.sock"
    }
},
{
...
project/
    backend/
        ...
        data/  "database files and clients"
            __init__.py/  "provides objects to access data"
    ...
    frontend/
        ...
        package.json
        vite.config.js
    readme/  "attachment files for README documentation"
    compose.yml  "example compose file" TODO: remove this file
